...
###
    ...
    def update(self):
        ...
        '''
        for i in xrange(0, self.sizepop):
            self.population[i].velocity = self.params[0] * self.population[i].velocity + self.params[1] * np.random.random_sample(self.vardim) * (self.population[i].bestPosition - self.population[i].chrom) + self.params[2] * np.random.random_sample(self.vardim) * (self.best.chrom - self.population[i].chrom)
...
#               self.population[i].chrom[j]=max(self.population[i].velocity#[j],0)                
            for j in xrange(0, self.vardim):
                self.population[i].chrom[j]=max(self.population[i].chrom[j],-1)
                self.population[i].chrom[j]=min(self.population[i].chrom[j],1)
                self.population[i].velocity[j]=max(self.population[i].velocity[j],-0.2)
                self.population[i].velocity[j]=min(self.population[i].velocity[j],0.2)
    ...
    def solve(self):
        ...
        while self.t < self.MAXGEN - 1:
            ...
            self.params[1]=-0.02*self.t+1.4962
            self.params[2]=0.01*self.t+1.4962
            self.evaluation()#评估。。。
            ...
            #np.save('result.npy',self.result)
